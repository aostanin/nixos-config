{
  ...
  # Pinned to linux 6.12 because mtk-pcie-gen3 regressed for this board on
  # 6.18: probe now runs at ~0.8s (vs ~5s in 6.12 where an upstream
  # EPROBE_DEFER in the clk/phy/regulator dep chain incidentally bought enough
  # head-room) and the LTSSM check fires before the NVMe is ready. The
  # PCIE_PORT_LINKUP poll times out at the spec'd 100ms (PCI_PM_D3COLD_WAIT)
  # and stage 1 can't mount the rootfs. OpenWRT carried patch 611 for 6.12
  # (extra 100ms between non-PERST and PERST# de-assert) and dropped it for
  # 6.18 claiming "merged upstream" — only the pre-deassert msleep landed,
  # not the inter-signal delay. A 6.18-rebased version of 611 doesn't help
  # either; the problem is *when* probe runs, not the reset timing.
  # Revisit when MediaTek/upstream/OpenWRT fix the probe ordering, or when
  # nixos-sbc lands a board-specific DT tweak.
  boot.kernelPackages = pkgs.linuxKernel.packagesFor (pkgs.linux_6_12.override {
    kernelPatches = [
      {
