  ...
  config = lib.mkIf config.router.enable {
    ...
    systemd.network = {
      ...
      networks = {
        ...
        # br-lan itself = LAN (VLAN 1) + parent of the vlanNN netdevs. No
        # static IPv6 /64 here — lan-prefix + dnsmasq + ndppd handle v6.
        "30-br-lan" = {
          matchConfig.Name = lan.interface;
          address = ["${lan.prefix}.1/24"];
        ...
        "31-guest" = {
          ...
          address = ["${guest.prefix}.1/24"];
          networkConfig.IPv4Forwarding = true;
        };

        "32-iot" = {
          matchConfig.Name = iot.interface;
          address = ["${iot.prefix}.1/24"];
          networkConfig.IPv4Forwarding = true;
        };
